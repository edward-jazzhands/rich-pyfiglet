...
@nox.parametrize("ver", MAJOR_RICH_VERSIONS)
def tests(session: nox.Session, ver: int) -> None:
    ...
    session.run("uv", "pip", "show", "rich")
    # EXPLANATION: The `ver + 1` is a trick to make UV
    # only download the last version of each major revision of Rich.
    # If the current version is 13, we're saying `install rich<14.0.0`.
    # This will make UV grab the highest version of Rich 13.x.x
    # The last `uv pip show rich` is just for logging purposes.
